...
try:
    from buyer_synthetic.config.settings import settings, SurveyMode
    ...
    from src.buyer_synthetic.config.settings import settings, SurveyMode
    from src.buyer_synthetic.utils.logger import get_logger
    from src.buyer_synthetic.agents.survey_creator import SurveyCreatorAgent
    from src.buyer_synthetic.agents.survey_executor import SurveyExecutorAgent
...
class BuyerSyntheticDashboard:
    ...
    def initialize_session_state(self):
        ...
        if 'survey_mode' not in st.session_state:
            st.session_state.survey_mode = SurveyMode.DEMO
    ...
    def render_sidebar(self):
        ...
        # Configuración de modo de encuesta
        st.sidebar.subheader("🚀 Modo de Encuesta")
        survey_mode = st.sidebar.selectbox(
            "Selecciona el modo",
            [SurveyMode.DEMO, SurveyMode.GENAI],
            index=0 if st.session_state.survey_mode == SurveyMode.DEMO else 1,
            help="Demo Mode: respuestas simuladas instantáneas\nGenAI Mode: respuestas generadas con IA"
        )
        
        # Actualizar session state
        st.session_state.survey_mode = survey_mode
        
        # Configuración de modelo (solo visible en GenAI Mode)
        if survey_mode == SurveyMode.GENAI:
            ...
            api_status = "🟢 Configurada" if settings.OPENAI_API_KEY else "🔴 No configurada"
            st.sidebar.info(f"API Key: {api_status}")
            
            if not settings.OPENAI_API_KEY:
                st.sidebar.warning("⚠️ Se requiere OPENAI_API_KEY para GenAI Mode")
        else:
        ...
        mode_emoji = "🎮" if survey_mode == "Demo Mode" else "🤖"
        mode_description = "Simulación rápida" if survey_mode == "Demo Mode" else "IA con LangGraph"
        
        st.sidebar.info(f"""
        **Modo**: {mode_emoji} {survey_mode}
        **Tipo**: {mode_description}
...
3. ¿Cuál es el principal problema del país?
   ...
   Opciones: Economía, Seguridad, Corrupción, Educación
        ...
        with col2:
            ...
            **Estimado**: ~{len(df) * 2} minutos
            ...
            if st.button("🚀 Ejecutar Encuesta Completa", type="primary", use_container_width=True):
                # Validar configuración para GenAI Mode
                if config["survey_mode"] == SurveyMode.GENAI and not settings.OPENAI_API_KEY:
                    st.error("❌ Se requiere OPENAI_API_KEY para usar GenAI Mode")
                    st.info("💡 Configura tu API key en el archivo .env o usa Demo Mode")
                else:
    ...
    def _execute_survey_with_progress(self, df, batch_size, config, questions):
        ...
        try:
            survey_mode = config.get("survey_mode", "Demo Mode")
            
            # Mostrar indicador de modo
            if survey_mode == "Demo Mode":
                mode_indicator.info("🎮 **Demo Mode**: Generando respuestas simuladas instantáneas")
            else:
                mode_indicator.info("🤖 **GenAI Mode**: Generando respuestas con IA usando LangGraph")
            ...
            for batch_num in range(total_batches):
                ...
                if survey_mode == "Demo Mode":
                    ...
                    batch_results = self._process_batch_genai(batch_df, questions, config)
            ...
            progress_bar.empty()
            status_text.empty()
            mode_indicator.empty()
            
            mode_emoji = "🎮" if survey_mode == SurveyMode.DEMO else "🤖"
            st.success(f"✅ Encuesta completada con {mode_emoji} {survey_mode}: {len(results_df)} respuestas generadas")
    ...
    def _process_batch_genai(self, batch_df, questions, config):
        """Procesar lote usando GenAI Mode con LangGraph"""
        results = []
        
        for _, profile in batch_df.iterrows():
            try:
                # Usar el agente LangGraph para generar respuestas
                ...
                # Generar respuestas con LangGraph
                result = executor.execute_individual_survey(profile.to_dict(), questions)
                results.append(result)
                ...
                error_result = {
                    ...
                    "nse": profile["nse"],
                    "region": profile["region"],
                    "error": f"GenAI Error: {str(e)}",
                    "survey_timestamp": datetime.now().isoformat()
                }
                results.append(error_result)
                st.warning(f"⚠️ Error procesando {profile['nombre']}: {str(e)}")
